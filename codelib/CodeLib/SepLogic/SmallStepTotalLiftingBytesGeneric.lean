...
namespace Wasm.SmallStep.TerminalGeneric

open Wasm
open Iris Iris.ProgramLogic Language.Notation
open Wasm.SepLogic Wasm.SmallStep

variable {hlc : outParam HasLC} {α Terminal : Type}
variable [WasmSmallStepGS hlc α] [TerminalView α Terminal]
local instance (priority := high) genericBytesTerminalLanguage :
    Language (Expr α) (MachineStore α) StepKind Terminal :=
  TerminalView.canonicalLanguage
local instance (priority := high) genericBytesTerminalIrisGS :
    @IrisGS_gen hlc (Expr α) Terminal (MachineStore α) StepKind
      genericBytesTerminalLanguage (WasmHeapGF α) :=
...
theorem twp_load8U
  ...
  iapply twp_lift_step_no_fork
    (@TerminalView.running_not_val α Terminal _ _)
  ...
  · ipureintro
    ...
    exact ⟨.running
        ⟨⟨params, localValues, .i32 byte.toUInt32 :: values⟩,
          code, arity, remainder, controls, calls⟩,
      store, [], ⟨rfl, _, rfl, by simpa [Hread] using Step.load8U (Wasm.SmallStep.memoryAddress?_i32_eq address) hbound⟩⟩
  ...
  have expectedStep : Step
    ...
    simpa [Hread] using (Step.load8U (α := α) (Wasm.SmallStep.memoryAddress?_i32_eq address) hbound)
  obtain ⟨rfl, hconfig⟩ :=
...
theorem twp_store8
  ...
  · ipureintro
    ...
    exact ⟨.running
      ...
      [], ⟨rfl, _, rfl, Step.store8 (Wasm.SmallStep.memoryAddress?_i32_eq address) hbound⟩⟩
  iintro %κ %e₂ %store₂ %forks %Hstep
  rcases Hstep with ⟨hforks, kind, hobs, wasmStep⟩
  change forks = [] at hforks
  subst forks
  ...
  have expectedStep : Step
    ...
    Step.store8 (Wasm.SmallStep.memoryAddress?_i32_eq address) hbound
  obtain ⟨rfl, hconfig⟩ :=
    step_deterministic expectedStep wasmStep
  have parts := Config.mk.inj hconfig
  have hexpr := parts.1
...
end Wasm.SmallStep.TerminalGeneric
